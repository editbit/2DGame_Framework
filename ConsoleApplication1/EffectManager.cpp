	...
	if (!effectList.empty())
	{
		...
		while( iter != effectList.end())
		{
			...
			iter->eImage->frameRender(hdc, iter->x - CAM->getX() - iter->eImage->getFrameWidth()/2,
				iter->y - CAM->getY() - iter->eImage->getFrameHeight()/2, iter->index, 0);
...
void EffectManager::generateEffect(const char * effectName, float x, float y)
{
	Effect e;
